...
public abstract class Ustensile {
	...
	//méthode afficherValeurTotale
		...
		//update
		public void update_global_data(int id ,double rayoncotelongeur ,int anne_de_fabrication ,int choix) {
			//First f = new First();
			con.Connect();
			...
			try {
				...
				if (choix == 1) {
					//assietteronde
					sql = "UPDATE `assietteronde` SET `rayon`=?,`anne_de_fabrication`=? WHERE `id`=?";
					messagesucces =" \t modification assietteronde avec succes";
				}else if (choix == 2) {
					//assiettecarree
					sql = "UPDATE `assiettecarree` SET `cote`=?,`anne_de_fabrication`=? WHERE `id`=?";
					messagesucces =" \t modification assiettecarree avec succes";
				}else if (choix == 3) {
					//cuillere
					sql = "UPDATE `cuillere` SET `longeur`=?,`anne_de_fabrication`=? WHERE `id`=?";
				...
				ps.setDouble(1, rayoncotelongeur);
		...
		abstract void afficherValeurTotale();
		
		abstract void update_global_data();
